    ...
    for(k = 0; k < n; k+=b)
  {
   ...
   clapack_dpotrf(CblasColMajor,CblasLower,mk-k,&C[k+k*n],n);
    for(i = k+b; i < n; i+=b)
    {
      mi = min(i+b,n);
      #pragma omp task
      cblas_dtrsm(CblasColMajor,CblasRight,CblasLower,CblasTrans,CblasNonUnit,mi-i,mk-k,1,&C[k+k*n],n,&C[i+k*n],n) ;
    }
    for(i = k+b; i < n; i+=b)
    {
      mi = min(i+b,n);
      for(j = k + b; j < (i-1); j+=b)
      {
        mj = min(j+b,n);
        #pragma omp task
        cblas_dgemm(CblasColMajor,CblasNoTrans,CblasTrans,mi-i,mj-j,mk-k,-1,&C[i+k*n],n,&C[j+k*n],n,1,&C[i+j*n],n);
      }
      #pragma omp task
