...
void quicksort( long begin, long end, vector<double>& v ) {
  if( begin<end ) {
    ...
    if( size<1000 ) {
      ...
      /**
      quicksort( begin, medio, v );
      quicksort( medio+1, end, v );
      **/
      parallel_invoke(QuicksortClass(begin,medio,v),QuicksortClass(medio+1,end,v));
    }
  } 
}

void quicksortLambda( long begin, long end, vector<double>& v ) {
  if( begin<end ) {
    long size = end-begin;
    if( size<1000 ) {
      sort(v.begin()+begin,v.begin()+end);
      ...
      quicksort( medio+1, end, v );
      **/
      parallel_invoke([&]{quicksortLambda(begin,medio,v);},
                      [&]{quicksortLambda(medio+1,end,v);});
    }
  } 
}
